    ...
    <div className="w-full min-h-[80vh] flex flex-col items-center justify-center px-4 py-20">
      <div className="w-full max-w-md card p-8 shadow-2xl relative">
        ...
        <div className="text-center mb-8 mt-2">
          ...
          <h1 className="text-3xl font-bold text-white mb-2 text-glow">Lost Access?</h1>
          <p className="text-sm text-gray-400">Enter your email and we'll send a secure terminal reset link.</p>
        </div>
        ...
        {!message ? (
          <form className="space-y-6" onSubmit={handleReset}>
            <div>
              <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1 ml-1">Email Terminal</label>
              <input 
                type="email" 
          ...
          <Link href="/login" className="btn-primary w-full py-4 text-xs font-black uppercase tracking-widest block text-center mt-4">
            Return to Login
          </Link>
        )}
        
        <p className="text-center text-sm text-gray-600 mt-10">
          Remembered? <Link href="/login" className="text-white hover:text-green-electric font-medium transition-colors">Abort & Login</Link>
        </p>
      </div>
    </div>
  );
