          ...
          {indicators.map((item, idx) => {
            ...
            return (
              <motion.div 
                ...
                className={`group relative overflow-hidden rounded-[20px] border border-white/5 bg-white/[0.02] p-6 transition-all ${!isLocked ? 'hover:bg-white/[0.04] hover:border-[#00B0FF]/30' : ''}`}
              >
                {/* Glow */}
                {!isLocked && (
                ...
                <div className={`transition-all duration-500 ${isLocked ? 'blur-[8px] select-none pointer-events-none opacity-40' : ''}`}>
                  <div className="flex justify-between items-start mb-6">
                     ...
                     <motion.a 
                      ...
                      className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-tight transition-all flex items-center gap-2 ${!isLocked ? 'bg-[#00B0FF]/10 hover:bg-[#00B0FF]/20 text-[#00B0FF] border border-[#00B0FF]/20' : 'bg-white/5 text-white/20 border border-white/5'}`}
                     >
                ...
                {isLocked && (
                  <div className="absolute inset-0 z-20 flex flex-col items-center justify-center p-6 text-center bg-navy-deep/20 backdrop-blur-[2px]">
                    <div className="w-12 h-12 rounded-full bg-[#00B0FF]/10 border border-[#00B0FF]/20 flex items-center justify-center mb-4 shadow-[0_0_30px_rgba(0,176,255,0.1)]">
                      <ShieldCheck className="w-6 h-6 text-[#00B0FF]" />
                    </div>
                    <h4 className="text-white font-bold text-sm mb-1">Premium Tool</h4>
                    <p className="text-[10px] text-white/40 mb-6 max-w-[180px]">Exclusive indicator for Pro members. Upgrade to unlock.</p>
                    <Link 
                      href="/dashboard/marketplace"
                      className="px-6 py-2 bg-[#00B0FF] text-white text-[11px] font-black rounded-full hover:scale-105 active:scale-95 transition-all shadow-[0_4px_15px_rgba(0,176,255,0.3)]"
                    >
                      GET PRO ACCESS
                    </Link>
                  </div>
